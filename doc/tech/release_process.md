...
如果想发布一个供内部或小范围测试的 Beta 版本（例如 `v1.8.1-beta1`），流程与正式版相同，但 CI 行为会有所区别：

1. **版本号格式**：所有配置文件中的版本号改为带 `-betaN` 的格式，例如 `"1.8.1-beta1"`。
2. **Tag 命名**：使用对应的完整名称打 tag，例如 `git tag v1.8.1-beta1`。
3. **自动化流程区别**：
   - **Tauri 桌面端**：会正常构建 macOS 和 Windows 的安装包并上传到 Actions Artifacts，但**不会自动创建 GitHub Release**。
   - **Docker 镜像**：会构建并推送对应 `-betaN` 标签的镜像，但**不会更新 `latest` 标签**，避免影响普通用户拉取。
