...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `APP_VERSION` | `package.json` 中的 `version` | 覆盖界面展示的软件版本号（如侧边栏、设置页的「当前版本」）。便于二次开发时自定义展示，不修改代码即可换版本号。 |

未设置时回退到代码内置版本；设置了 `APP_VERSION`（如在 `.dev.vars` 或部署平台的环境变量中）则优先使用它。该变量只影响**展示**，不影响更新检查逻辑（更新比较仍使用代码内置版本，避免自定义版本触发误报「发现新版本」）。

```bash
# .dev.vars
APP_VERSION=dev-20260821
```
